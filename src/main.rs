...
fn apply_env(config: &mut Config) {
    ...
    if let Ok(raw) = std::env::var("MAILINER_ALLOWED_ORIGINS") {
        config.allowed_origins = parse_csv_set(&raw);
    }
    if let Ok(raw) = std::env::var("MAILINER_ALLOWED_HOSTS") {
        config.allowed_hosts = parse_csv_set(&raw);
    }
